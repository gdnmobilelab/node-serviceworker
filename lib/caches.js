...
function CacheEntry(response, url, defaultTTL) {
...
CacheEntry.prototype._calcTTL = function _calcMaxAge(response, defaultTTL) {
    // Figure out maxAge
...
CacheEntry.prototype._extractVary = function _extractVary(response) {
 ...
 * @return {boolean}
 */
CacheEntry.prototype.matchEntry = function matchEntry(otherEntry) {
    var self = this;
    // vary
    if (!this.vary) {
        return true;
 ...
 * @return {boolean}
 */
CacheEntry.prototype.matchRequest = function matchRequest(request) {
    var self = this;
    // vary
    if (!this.vary) {
        return true;
...
Cache.prototype.put = function(url, request) {
    var entry = new CacheEntry(request, url, this.options.defaultTTL);
    if (entry.ttl > 0) {
        var entries = this._map.get(url);
        if (!entries) {
            entries = [];
...
Cache.prototype.matchAll = function(request) {
    var candidates = this._map.get(request.url);
    if (candidates) {
        var matches = [];
        var now = Date.now();
