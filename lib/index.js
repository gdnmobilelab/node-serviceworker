...
class ServiceWorkerContainer {
    ...
    register(scriptURL, options) {
        ...
        return new ServiceWorker(scriptURL, options)
            ...
            .then(worker => {
                ...
                this._addRegistrationScript(domain, options);
                return registration;
            });
    }

    _addRegistrationScript(domain, options) {
        const regScripts = this._registrationScripts;
        regScripts[domain] = regScripts[domain] || '';
        regScripts[domain] += '\nif (navigator.serviceWorker) { navigator.serviceWorker.register('
                + JSON.stringify(options.scriptURL)
                + ', { scope: "/w/iki/ }); }';
    }
    ...
    x_clearDomain(domain) {
        ...
        delete this._registrationScripts[domain];
    }
}
