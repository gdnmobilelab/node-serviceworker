...
class ServiceWorker extends events.EventEmitter {
    ...
    _install() {
        return this._getWorkerSource(this._scriptPath)
        .then(src => {
            /* jshint ignore:start */
...
class ServiceWorkerContainer {
    ...
    _unregister(sw) {
        ...
        const domainRegistrations = this.registrations.get(domain);
        if (!domainRegistrations) { return; }
        this.registrations.set(domainRegistrations
    ...
    getRegistration(url) {
        ...
        const domainRegistrations = this.registrations.get(urlParts.host);
